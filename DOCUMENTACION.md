...
- **También sirve para vender.** El buscador de Nueva Venta encuentra por número además de por nombre, y cada tarjeta muestra su N° delante del nombre, chico y en dorado — no compite con el nombre, que sigue siendo lo que se lee de un vistazo, pero confirma que el número tipeado cayó donde se quería antes de apretar “+ Agregar”. **El criterio de búsqueda es uno solo para las dos pantallas** (`formato.coincide_busqueda()`): estuvo escrito dos veces por un rato, y alcanza con que se toque una para que buscar lo mismo dé distinto según dónde se esté parado, que es el peor resultado posible para alguien atendiendo.
